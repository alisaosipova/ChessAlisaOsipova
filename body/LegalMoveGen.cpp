...
MoveList LegalMoveGen::generate(const Position& position, uint8_t side, bool onlyCaptures) {
	...
	while (allKnights) {
		attacketP = BOp::bsf(allKnights);
		allKnights = BOp::set0(allKnights, attacketP);
		mask = PsLegalMoveMaskGen::generateKnightMask(position.getPieces(), attacketP, side, onlyCaptures);
		pieceMaskToMoves(position.getPieces(), mask, attacketP, PIECE::KNIGHT, side, moves);
	}
	while (allBishops) {
		attacketP = BOp::bsf(allBishops);
		allBishops = BOp::set0(allBishops, attacketP);
		mask = PsLegalMoveMaskGen::generateBishopMask(position.getPieces(), attacketP, side, onlyCaptures);
		pieceMaskToMoves(position.getPieces(), mask, attacketP, PIECE::BISHOP, side, moves);
	}
	while (allRooks) {
		attacketP = BOp::bsf(allRooks);
		allRooks = BOp::set0(allRooks, attacketP);
		mask = PsLegalMoveMaskGen::generateRookMask(position.getPieces(), attacketP, side, onlyCaptures);
		pieceMaskToMoves(position.getPieces(), mask, attacketP, PIECE::ROOK, side, moves);
	}
	while (allQueens) {
		attacketP = BOp::bsf(allQueens);
		allQueens = BOp::set0(allQueens, attacketP);
		mask = PsLegalMoveMaskGen::generateQueenMask(position.getPieces(), attacketP, side, onlyCaptures);
		pieceMaskToMoves(position.getPieces(), mask, attacketP, PIECE::QUEEN, side, moves);
	}
	attacketP = BOp::bsf(position.getPieces().getPieceBitboard(side, PIECE::KING));
	mask = PsLegalMoveMaskGen::generateKingMask(position.getPieces(), attacketP, side, onlyCaptures);
	pieceMaskToMoves(position.getPieces(), mask, attacketP, PIECE::KING, side, moves);
